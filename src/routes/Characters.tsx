import type { HTMLAttributes, PropsWithChildren } from "react";
import { useLoaderData, useActionData } from "react-router-dom";

type CharactersProps = {} & HTMLAttributes<HTMLDivElement>;

export default function Characters({
  ...rest
}: PropsWithChildren<CharactersProps>) {
  useLoaderData();
  useActionData();

  return (
    <div {...rest} className={`p-6 w-full h-full overflow-auto flex flex-col ${rest.className || ''}`}>
      <div className="flex justify-between items-center mb-6 shrink-0">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Characters</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Manage and view story characters.
          </p>
        </div>
      </div>

      <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-sm border border-slate-200 dark:border-slate-700 text-center">
        <div className="w-16 h-16 bg-primary/10 text-primary rounded-2xl flex items-center justify-center mx-auto mb-4">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6 0 3.375 3.375 0 0 1 6 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
          </svg>
        </div>
        <p className="text-slate-600 dark:text-slate-300 font-medium">Characters page connected.</p>
        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
          Loader and action routes are connected and ready for character data.
        </p>
      </div>
    </div>
  );
}

